...
def downloadsocks(choice):
    ...
    if choice == "1":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://pastebin.com/raw/aYsFkANZ",
                             timeout=5)
            f.write(r.content)
        except:
            pass
        try:
            r = requests.get("https://pastebin.com/raw/aYsFkANZ", timeout=5)
            f.write(r.content)
            f.close()
        except:
            pass
        try:
            r = requests.get("https://pastebin.com/raw/fsUp3cRC", timeout=5)
            f.write(r.content)
            f.close()
        except:
            pass
